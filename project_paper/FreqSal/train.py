...
import torch,gc
from CFL import CoFocusFrequencyLoss


gc.collect()  # clean cache,save memory
torch.cuda.empty_cache()

cfl = CoFocusFrequencyLoss(loss_weight=1, alpha=1, patch_factor=1, ave_spectrum=True, log_matrix=True, batch_matrix=True).cuda()
...
logging.basicConfig(filename=save_path + 'FreqSal.log',
                    format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]', level=logging.INFO, filemode='a',
                    datefmt='%Y-%m-%d %I:%M:%S %p')
logging.info("FreqSal-Train_4_pairs")
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, gts, depth, edge) in enumerate(train_loader, start=1):
            ...
            s, e, s1, s2, s3, s4, r1, t1 = model(images, depth)

            sal1_loss = bce_iou_loss(s1, gts)
            sal2_loss = bce_iou_loss(s2, gts)
            sal3_loss = bce_iou_loss(s3, gts)
            sal4_loss = bce_iou_loss(s4, gts)
            sal_loss = bce_iou_loss(s, gts)

            edge_loss1 = CE(e, edge)
            edge_loss2 = cfl(e, edge, r1, t1)
            edge_loss = edge_loss1 + edge_loss2
            loss = sal_loss + sal1_loss + sal2_loss + sal3_loss + sal4_loss + edge_loss
            loss.backward()
        ...
        if (epoch) % 50 == 0:
            torch.save(model.state_dict(), save_path + 'FreqSal_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'FreqSal_epoch_{}.pth'.format(epoch + 1))
...
def test(test_loader, model, epoch, save_path):
    global best_mae, best_epoch, best_model_count
    model.eval()
    with torch.no_grad(): # aim to save memory
        ...
        for i in range(test_loader.size):
            ...
            image = image.cuda()
            depth = depth.repeat(1,3,1,1).cuda()
            res, e, s1, s2, s3, s4, r1, t1 = model(image,depth)
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + f'FreqSal_epoch_best_{best_model_count}.pth')
                best_model_count += 1  # Increment the counter
                print('Saved best model: FreqSal_epoch_best_{}.pth'.format(best_model_count - 1))
